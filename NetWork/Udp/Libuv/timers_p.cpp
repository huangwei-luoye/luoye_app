...
uint64_t calculateNextTimeout(TimerInfo* info, const struct timeval& now)
{
    ...
    if (info->interval)
    {
        ...
        if (((info->interval < 1000) && ((twhen - tnow) > 1500))
...
void QEventDispatcherLibuvPrivate::registerTimer(int timerId, int interval, Qt::TimerType type, QObject* object)
{
    Q_ASSERT(interval > 0);
    ...
    info->ev.data = info;
    uv_timer_start(tmp, QEventDispatcherLibuvPrivate::timer_callback, delta, 0);
    this->m_timers.insert(timerId, info);
}

void QEventDispatcherLibuvPrivate::registerZeroTimer(int timerId, QObject* object)
{
    ZeroTimer data;
    data.object = object;
    data.active = true;
...
bool QEventDispatcherLibuvPrivate::unregisterTimer(int timerId)
{
...
bool QEventDispatcherLibuvPrivate::unregisterTimers(QObject* object)
{
    bool result = false;
...
QList<QAbstractEventDispatcher::TimerInfo> QEventDispatcherLibuvPrivate::registeredTimers(QObject* object) const
{
...
int QEventDispatcherLibuvPrivate::remainingTime(int timerId) const
{
...
void QEventDispatcherLibuvPrivate::timer_callback(
    uv_timer_t* w
...
{
    QEventDispatcherLibuvPrivate* self = static_cast<QEventDispatcherLibuvPrivate*>(w->loop->data);
...
bool QEventDispatcherLibuvPrivate::disableTimers(bool disable)
{
    struct timeval now;
    if (!disable)
    {
...
void QEventDispatcherLibuvPrivate::killTimers(void)
{
