...
QEventDispatcherLibuvPrivate::QEventDispatcherLibuvPrivate(QEventDispatcherLibuv* const q)
      ...
      m_notifiers(), m_timers(), m_event_list(), m_zero_timers(), m_awaken(false)
{
    ...
    uv_async_init(this->m_base,
                  &this->m_wakeup,
                  QEventDispatcherLibuvPrivate::wake_up_handler);
}

QEventDispatcherLibuvPrivate::~QEventDispatcherLibuvPrivate(void)
{
    if (this->m_base)
    {
...
bool QEventDispatcherLibuvPrivate::processEvents(QEventLoop::ProcessEventsFlags flags)
{
    Q_Q(QEventDispatcherLibuv);
...
bool QEventDispatcherLibuvPrivate::processZeroTimers(void)
{
    bool result    = false;
...
void QEventDispatcherLibuvPrivate::wake_up_handler(
    uv_async_t* w
#if UV_VERSION_MAJOR < 1
    , int
#endif
...
{
    QEventDispatcherLibuvPrivate* disp = static_cast<QEventDispatcherLibuvPrivate*>(w->loop->data);
    disp->m_awaken = true;
